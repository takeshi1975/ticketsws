...
public class TestInformacion {
	...
	public void testInformacion() {
		...
		ReservaCerrarRespuesta respuesta = restTemplate.postForObject("/tickets/purchase", purchase,
				ReservaCerrarRespuesta.class);
		assert respuesta != null;
		logger.info("Localizador de la compra " + respuesta.getLocata());
		String  recibo= restTemplate.postForObject("/tickets/voucher/orderCode/"+respuesta.getLocata(),null, String.class);
		assert recibo!=null;
		logger.info(recibo);		
	}
